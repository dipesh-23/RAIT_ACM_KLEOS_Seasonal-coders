...
class DatabaseService {
  ...
  Future<Database> _initDb() async {
    ...
    return await openDatabase(
      ...
      onUpgrade: (db, oldVersion, newVersion) async {
        ...
        await db.execute('DROP TABLE IF EXISTS pregnancy_profiles');
        await db.execute('DROP TABLE IF EXISTS pregnancy_visits');
        await db.execute('DROP TABLE IF EXISTS followup_records');
        await db.execute('DROP TABLE IF EXISTS epidemic_snapshots');
  ...
  Future<List<SessionModel>> getRecentSessions(String workerName, int limit) async {
    final db = await database;
    final maps = await db.query('sessions',
        where: 'asha_worker_name = ?',
        whereArgs: [workerName],
